  ...
  return (
    <AppShell role={user.role} user={toAppShellUser(user)}>
      <div className="space-y-6">
        ...
        {user.role !== "ADMIN" ? (
          <Card><CardContent className="pt-5 text-sm text-muted-foreground">Admin access required.</CardContent></Card>
        ) : (
          <>
            <div className="grid gap-4 lg:grid-cols-2">
              <Card>
                <CardHeader><CardTitle>Seed data helper</CardTitle></CardHeader>
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                  <p>Run <code className="font-mono text-foreground">npm run seed</code> after migrations to add demo templates and a sample evaluation.</p>
                  <p>Seed records are idempotent and keep editable product examples out of page components.</p>
                </CardContent>
              </Card>
              <Card>
                <CardHeader><CardTitle>AI model config overview</CardTitle></CardHeader>
            ...
            <Card>
              <CardHeader><CardTitle>Usage events</CardTitle></CardHeader>
              <CardContent>
                {usage.length === 0 ? (

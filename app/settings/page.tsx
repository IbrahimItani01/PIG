          ...
          <Card>
            ...
            <CardContent className="space-y-4 text-sm">
              ...
              <div className="space-y-3">
                <div><span className="text-muted-foreground">Auth provider:</span> <Badge variant="secondary">{user.authProvider}</Badge></div>
                <div><span className="text-muted-foreground">Email confirmed:</span> {user.emailConfirmedAt ? "Yes" : "Pending"}</div>
                <div><span className="text-muted-foreground">Last sign-in:</span> {user.lastSignInAt ? new Date(user.lastSignInAt).toLocaleString() : "Not available"}</div>
